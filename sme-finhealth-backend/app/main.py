...
# register extra models so Base.metadata.create_all() creates all tables
from .models_invoice import Invoice
from .models_rbac import BusinessUser


app = FastAPI(title="SME Financial Health Backend", version="1.0.0")
...
async def upload_preview(
    ...
    return {
        ...
        "detected": {"rows": int(norm.shape[0])},
    }
...
async def pdf_preview(
    ...
    if df.empty:
        return {
            "columns": [],
            "preview": [],
            "detected": {"rows": 0, "message": "No rows detected (is this a text PDF?)"},
        }
...
async def pdf_commit(
    ...
    if df.empty:
        raise HTTPException(status_code=400, detail="No transactions detected from PDF")
...
def kpis(
    ...
    revenue = sum(
        float(t.amount)
        for t in txns
        if (t.category or "").lower() == "revenue" and (t.direction or "").lower() == "credit"
    )
...
def forecast(
    ...
    for i in range(1, months + 1):
        ...
        out.append(
            {
                "month": m,
                "forecast_revenue": round(avg_rev, 2),
                "forecast_net_cashflow": round(avg_profit, 2),
            }
        )
    ...
    return {
        "business_id": business_id,
        "forecast": out,
        "based_on_months": [x["month"] for x in last_n],
    }
